...
AR_data <- full_join(AR_inf,AR_eff)
...
p2 <- ggplot() + 
  geom_polygon(data = EU_map, aes(x=long, y = lat, group = group, fill=inf_mean), color="grey40") + 
  scale_y_continuous(limits = c(34,71)) + 
  theme(panel.grid = element_blank(), panel.background = element_blank(), 
        plot.title = element_text(size=20, face = "bold")) +
  labs(x="Longitude", y="Latitude", title="B") + scale_fill_gradient(name="Influent\nAR prevalence", high="red", low="darkgreen", na.value = "grey70")

p3 <- ggplot() + 
  geom_polygon(data = EU_map, aes(x=long, y = lat, group = group, fill=eff_mean), color="grey40") + 
  scale_y_continuous(limits = c(34,71)) + 
  theme(panel.grid = element_blank(), panel.background = element_blank(), 
        plot.title = element_text(size=20, face = "bold")) +
  labs(x="Longitude", y="Latitude", title="C") + scale_fill_gradient(name="Effluent\nAR prevalence", high="red", low="darkgreen", na.value = "grey70")
